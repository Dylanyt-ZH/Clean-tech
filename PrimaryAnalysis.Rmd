...
for (keyword in topics){
    ...
    ggsave(paste0(outputpath,keyword,' Average cititation in Top GDP Country.png'),plot = p4)
...
for (keyword in topics){
    ...
    p2 <- ggplot(data=rnd_rate,aes(x = country, y= rate)) + 
      geom_bar(stat = 'identity') +
      geom_text(aes(label = round(rate,2)), vjust = 1.5, colour = "white") +
      labs(title = ' RND rate in SAEs', x= 'Country',y = 'Paper Åmount / R&D(billion US dollar)',subtitle = keyword) +
      theme(plot.title = element_text(hjust = 0.5))
    ...
    ggsave(paste0(outputpath,keyword,' Average cititation in SAEs.png'),plot = p4)
}
