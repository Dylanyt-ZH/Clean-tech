...
results <- GDP_data[,c(3:23)] * RND_data_pp[,c(3:23)]
...
unique(uni_selected[which(uni_selected$affiliation.country == 'Norway'),'affilname'])
```
...
for (keyword in topics){
    ...
    p1 <- ggplot(data=country_gdp_rate,aes(x = country, y= rate)) + 
      geom_bar(stat = 'identity') + 
      geom_text(aes(label = round(rate,2)), vjust = 1.5, colour = "white") + 
      labs(title = ' GDP rate in top GDP Country',x='Country', subtitle = keyword,y = 'Paper Amount / GDP(billion US dollor)') +
      theme(plot.title = element_text(hjust = 0.5))
    ...
    p2 <- ggplot(data=rnd_rate,aes(x = country, y= rate)) + 
      geom_bar(stat = 'identity') +
      geom_text(aes(label = round(rate,2)), vjust = 1.5, colour = "white") +
      labs(title = ' RND rate in Top GDP countries', x= 'Country',y = 'Paper Åmount / RND(billion US dollor)',subtitle = keyword) +
      theme(plot.title = element_text(hjust = 0.5))
    ...
    p4 <- ggplot(data=avg_cite,aes(x = country, y= avg_count)) + 
      ...
      labs(title = ' Average cititation num in Top GDP countries', x= 'Country',y = 'Average cititation num',subtitle = keyword) +
      theme(plot.title = element_text(hjust = 0.5))
    print(p1)
    print(p2)
    print(p3)
...
for (keyword in topics){
    ...
    p1 <- ggplot(data=country_gdp_rate,aes(x = country, y= rate)) + 
      geom_bar(stat = 'identity') + 
      geom_text(aes(label = round(rate,2)), vjust = 1.5, colour = "white") + 
      labs(title = ' GDP rate in SAEs',x='Country', subtitle = keyword,y = 'Paper Amount / GDP(billion US dollor)') +
      theme(plot.title = element_text(hjust = 0.5))
    ...
    p2 <- ggplot(data=rnd_rate,aes(x = country, y= rate)) + 
      geom_bar(stat = 'identity') +
      geom_text(aes(label = round(rate,2)), vjust = 1.5, colour = "white") +
      labs(title = ' RND rate in SAEs', x= 'Country',y = 'Paper Åmount / RND(billion US dollor)',subtitle = keyword) +
      theme(plot.title = element_text(hjust = 0.5))
    ...
    p4 <- ggplot(data=avg_cite,aes(x = country, y= avg_count)) + 
      geom_bar(stat = 'identity') + 
      geom_text(aes(label = round(avg_count,2)), vjust = 1.5, colour = "white") + 
      labs(title = ' Average cititation num in SAEs', x= 'Country',y = 'Average cititation num',subtitle = keyword) +
      theme(plot.title = element_text(hjust = 0.5))
    print(p1)
    print(p2)
    print(p3)
...
ggplot(GDP_Paper, aes(x=reorder(Country.Name,avg_gdp))) +
  geom_bar(aes(y=avg_gdp),stat = 'identity', fill = 'grey',group = 1) + 
  geom_line(aes(y=paper_amount/4.5),color = 'red',group = 2) + 
  scale_y_continuous(name = "GDP (2000-2021, Billion US Dollor)",sec.axis = sec_axis(~.*4.5, name="Paper Amount (All Keywords)")) +
  coord_flip() +
...
ggplot(RND_Paper, aes(x=reorder(Country.Name,avg_rnd))) +
  geom_bar(aes(y=avg_rnd),stat = 'identity', fill = 'grey',group = 1) + 
  geom_line(aes(y=paper_amount/1.6),color = 'red',group = 2) + 
  scale_y_continuous(name = "RND (2000-2020, Billion US Dollor)",sec.axis = sec_axis(~.*1.6, name="Paper Amount (All Keywords)")) +
  coord_flip() +
  labs(title = 'GDP vs Paper Amount',x = 'Country') +
  theme(axis.title.x.top = element_text(color = "red"),axis.text.x.top = element_text(color = 'red'))
